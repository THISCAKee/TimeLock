...
The `sessions` table remains unchanged. Session history already stores the username and session details independently, so historical entries remain available after their corresponding `users` row is removed. SQLite foreign-key enforcement must not cause user deletion to remove or reject historical session rows; the implementation and tests will verify the current schema behavior explicitly.
